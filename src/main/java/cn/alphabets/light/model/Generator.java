...
public class Generator {
    ...
    public void generate(String domain, List<String> schema) {
        ...
        List<Document> defines = model.list(condition, Arrays.asList("schema", "parent", "items"));

        // 遍历表定义，生成对应的Entity类代码
        defines.forEach((define) -> {

            // 生成类定义
            final TypeSpec.Builder builder = this.mainClass(define.getString("parent"),
                    define.getString("schema"));

            // 遍历所有字段
            Document items = (Document) define.get("items");
            items.forEach((key, value) -> {

                // 跳过共通的字段（共通字段在父类里已经定义）
                if (!isCommonField(key)) {

                    // 生成字段的定义
                    this.subClass(builder, key, (Document) value);
                }
            });
    ...
    private boolean isCommonField(String name) {
        for (Field field : (ModCommon.class).getDeclaredFields()) {
    ...
    private void subClass(TypeSpec.Builder spec, String name, Document define) {
        ...
        items.forEach((key, value) -> this.subClass(builder, key, (Document) value));
